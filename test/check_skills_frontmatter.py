...
def check_skill_frontmatter(file_path, verbose=False):
    ...
    try:
        ...
        if 'name' in data:
            ...
            if name != folder_name and name != f"secops-{folder_name}" and not is_copy:
                errors.append(f"Skill name '{name}' does not match folder name '{folder_name}' or 'secops-{folder_name}'")
